...
class Decoder(nn.Module):
    def __init__(self, in_ch, concat_ch, out_ch, mid_ch=None):
        ...
        self.ConcatConv = DoubleConv(in_ch + concat_ch, out_ch, mid_ch)
        self.CBAM = CBAM(out_ch)
...
#         super().__init__()
#         self.conv_x = nn.Conv2d(x_channels, g_channels, 1, bias=False)
#         self.conv_g = nn.Conv2d(g_channels, g_channels, 1, bias=False)
#         self.relu = nn.ReLU(inplace=True)
#         self.conv_psi = nn.Conv2d(g_channels, 1, 1, bias=True)
#         self.sigmoid = nn.Sigmoid()

#     def forward(self, x, g):
#         # x: encoder feature, g: decoder feature (after up)
#         theta_x = self.conv_x(x)
#         phi_g = self.conv_g(g)
#         psi = self.relu(theta_x + phi_g)
#         att = self.sigmoid(self.conv_psi(psi))  # [B,1,H,W]
#         return x * att
...
class CBAM_UNet(nn.Module):
    ...
    def __init__(self, in_ch=1, out_ch=2, bilinear=True, base_c=64):
        ...
        self.bridge = DoubleConv(base_c * 16, base_c * 16)
...
if __name__ == '__main__':
    model = CBAM_UNet(in_ch=1, out_ch=2, bilinear=True, base_c=64).to('cuda')
    print("-" * 30)
    print(model)
